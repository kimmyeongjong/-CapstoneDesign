...
//auth()->user()->id
    ...
    public function location($select){
        ...
        for($i=0;$i<count($arr); $i++) {
            $value[$i] = RegisterDevice::join('measurements', 'register_devices.device_id', '=', 'measurements.device_id')
                                    ->select('' . $select . ' as data', 'latitude', 'longitude', 'register_devices.device_id', 'details_name',
                                        DB::raw('(CASE
                                    ...
                                    ->where('register_devices.device_id',$arr[$i]['device_id'])
    ...
    public function myDevice($select){
        ...
        if($error_cnt>0){
            ...
            $normal =  RegisterDevice::join('measurements', 'register_devices.device_id', '=', 'measurements.device_id')
                ->select('measurements.device_id')
                ->where('user_id',1)
                ->whereNotIn('measurements.device_id', $err_num)
                ->groupBy('measurements.device_id')
                ->orderBy($select,'desc')
                ->get();
